...
class EatingClubs: UIViewController, UITableViewDelegate, UITableViewDataSource, AVSpeechSynthesizerDelegate {
...
//        utterance.rate = 0.4
    ...
    func loadSavedData() {
        // 1
        guard let fileName = Bundle.main.path(forResource: "EatingClubs" ,ofType: "json")
            else { return }
...
extension EatingClubs: MKMapViewDelegate {
    //        // 1
...
extension EatingClubs: CLLocationManagerDelegate {
...
extension EatingClubs: HandleMapSearch {
    func dropPinZoomIn(placemark:MKPlacemark){
        // cache the pin
